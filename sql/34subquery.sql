...
SELECT SupplierID ,
(SELECT COUNT(*) FROM customers c WHERE c.Country = s.Country ) AS `Number Of Customers`
FROM suppliers s;
...
SELECT LastName,
       (SELECT COUNT(*) FROM employees e2 WHERE e1.BirthDate>e2.BirthDate) NumbrOfEmp
FROM employees e1;
